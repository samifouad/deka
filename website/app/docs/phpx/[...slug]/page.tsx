...
export async function generateStaticParams() {
  const slugs = getAllRuntimeDocSlugs('phpx')
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  const doc = getRuntimeDoc('phpx', resolvedSlug, docs) || (resolvedSlug.length === 1
    ? getRuntimeDoc('phpx', [resolvedSlug[0], 'index'], docs)
    : null)
  ...
  return {
    title: `${doc.metadata.title} | Deka PHPX Runtime`,
    description: doc.metadata.description,
  }
}

export default async function PHPXRuntimeDocPage({ params }: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await params
  ...
  const breadcrumbItems = [
    ...
    { label: 'phpx', href: buildDocsPath('phpx', version) },
  ]

  if (docSlug.length > 1) {
    breadcrumbItems.push({ label: docSlug[0], href: buildDocsPath('phpx', version, [docSlug[0]]) })
  } else if (docSlug.length === 1) {
    breadcrumbItems.push({
      label: docSlug[0],
      href: buildDocsPath('phpx', version, [docSlug[0]]),
      linkCurrent: true,
    })
  }
  ...
  const categoryItems = isCategoryPage
    ? getRuntimeCategoryItems('phpx', docSlug[0], docs)
    : []
          ...
          <div className="mt-12 border-t border-border/40 pt-8">
            ...
            <div className="space-y-3">
              {categoryItems.map((item) => (
                <Link
                  key={item.slug}
                  href={buildDocsPath('phpx', version, item.slug.split('/'))}
