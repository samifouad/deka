...
**When this document says "stdio"**, it refers to the **OS-level standard streams concept** (file descriptors):
- `stdin` = file descriptor 0 (standard input)
- `stdout` = file descriptor 1 (standard output)
- `stderr` = file descriptor 2 (standard error)
- `fd 3` = file descriptor 3 (used for IPC channel)

**This is NOT the `deka-stdio` crate** (which is for logging/formatting output).

To avoid confusion, this document will use:
- **"file descriptors"** or **"fd 0/1/2/3"** when referring to OS streams
- **"standard streams"** when referring to stdin/stdout/stderr
- **"`deka-stdio` crate"** when referring to the logging library
...
- ✓ `stdio: 'inherit'` support (inherit parent's stdin/stdout/stderr)
- ✓ Environment variable passing
- ✓ Fork delegates to Node.js (temporary workaround)

### What's Missing ✗
- ✗ IPC channel setup (file descriptor 3 / fd 3)
...
1. Creates an extra communication channel at **file descriptor 3** (fd 3)
2. Standard file descriptors:
   - fd 0 = stdin (standard input)
   - fd 1 = stdout (standard output)
   - fd 2 = stderr (standard error)
   - **fd 3 = IPC channel** (bidirectional message pipe)
