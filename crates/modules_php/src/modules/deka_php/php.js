...
const { op_php_get_wasm, op_php_parse_phpx_types, op_php_read_file_sync, op_php_write_file_sync, op_php_mkdirs, op_php_sha256, op_php_read_env, op_php_db_call, op_php_db_call_proto, op_php_db_proto_encode, op_php_db_proto_decode, op_php_net_call, op_php_fs_call, op_php_fs_call_proto, op_php_fs_proto_encode, op_php_fs_proto_decode, op_php_cwd, op_php_file_exists, op_php_path_resolve, op_php_read_dir, op_php_parse_wit } = Deno.core.ops;
const console = {
    log: (...args)=>{
...
let __phpEnv = {};
try {
    if (globalThis.Deno && globalThis.Deno.env && typeof globalThis.Deno.env.toObject === 'function') {
        __phpEnv = globalThis.Deno.env.toObject() || {};
    }
} catch (_err) {}
const __phpOpEnv = op_php_read_env();
if (__phpOpEnv && typeof __phpOpEnv === 'object') {
    __phpEnv = {
        ...__phpEnv,
        ...__phpOpEnv
    };
...
function writeDekaLock(lockPath, lock) {
    ...
    globalThis.fs.writeFileSync(lockPath, json, 'utf8');
}
...
function phpWasmCall(modulePtr, moduleLen, exportPtr, exportLen, argsPtr, argsLen) {
    try {
        ...
        if (isDbBridge || isNetBridge || isFsBridge || isLegacyHostBridge) {
            let payload = null;
            if (argsJson) {
                try {
            ...
            try {
                if (isDbBridge || moduleId.includes('db')) {
                    result = routeHostCall('db', String(exportName || ''), payload);
                } else if (isNetBridge || moduleId.includes('net')) {
                    result = op_php_net_call(String(exportName || ''), payload);
                } else if (isFsBridge || moduleId.includes('fs')) {
                    result = op_php_fs_call(String(exportName || ''), payload);
                } else {
                    result = {
                        ok: false,
