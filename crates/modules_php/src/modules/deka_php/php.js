...
const { op_php_get_wasm, op_php_parse_phpx_types, op_php_read_file_sync, op_php_write_file_sync, op_php_read_env, op_php_cwd, op_php_file_exists, op_php_path_resolve, op_php_read_dir, op_php_parse_wit } = Deno.core.ops;
const console = {
    log: (...args)=>{
...
function encodeModuleIdForPath(moduleId) {
    let out = '';
    for (let i = 0; i < moduleId.length; i++){
        const ch = moduleId[i];
        const code = moduleId.charCodeAt(i);
        const isAlpha = code >= 65 && code <= 90;
        const isLower = code >= 97 && code <= 122;
        const isDigit = code >= 48 && code <= 57;
        if (isAlpha || isLower || isDigit || ch === '_' || ch === '-' || ch === '.') {
            out += ch;
        } else {
...
function writeModuleCache(cacheRoot, module) {
    if (!cacheRoot) return '';
    const fileName = `.phpx-cache-${encodeModuleIdForPath(module.moduleId)}.phpx`;
    const outPath = globalThis.path.resolve(cacheRoot, fileName);
    globalThis.fs.writeFileSync(outPath, module.code, 'utf8');
    return outPath;
}
function buildLazyModuleRegistry(modules, entryPath) {
    ...
    for (const module of modules.values()){
        ...
        const cachePath = writeModuleCache(cacheRoot, module);
        if (deps.length === 0) {
            out += `__phpx_register_lazy('${escapePhpString(module.moduleId)}', '${escapePhpString(cachePath)}', array());\n`;
        } else {
            const depsList = deps.map((dep)=>`'${escapePhpString(dep)}'`).join(', ');
            out += `__phpx_register_lazy('${escapePhpString(module.moduleId)}', '${escapePhpString(cachePath)}', array(${depsList}));\n`;
        }
    }
    return out;
}
...
function buildPhpxBridgePrelude() {
    ...
    out += "  else if (isset($entry['code'])) { eval($entry['code']); }\n";
