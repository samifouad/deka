...
const { op_php_get_wasm, op_php_read_file_sync, op_php_read_env, op_php_cwd, op_php_file_exists, op_php_path_resolve, op_php_read_dir, op_php_parse_wit } = Deno.core.ops;
const console = {
    log: (...args)=>{
...
function buildPhpImportWrappers(imports) {
    let out = '';
    const seen = new Set();
    for (const spec of imports){
        if (spec.kind === 'wasm') continue;
...
function parsePhpxModule(filePath, modulesRoot) {
    ...
    const registryCode = buildExportRegistry(moduleId, mergedExports, moduleNamespace);
    return {
        moduleId,
        filePath,
        imports,
...
function buildExportRegistry(moduleId, exports, moduleNamespace) {
    ...
    for (const name of exports){
        ...
        out += `  '${escapedName}' => '${moduleNamespace}\\\\${name}',\n`;
    }
    out += " );\n";
    return out;
}
...
function buildPhpxBridgePrelude() {
    ...
    out += "  if (!array_key_exists($name, $exports)) { return __phpx_fail('Unknown phpx export: ' . $moduleId . ':' . $name); }\n";
...
function buildModulePrelude(entryPath) {
    ...
    if (phpxImports.length > 0) {
        const wrappers = buildPhpImportWrappers(phpxImports);
        if (isPhpxEntry) {
