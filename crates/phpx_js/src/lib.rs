...
pub fn compile_phpx_source_to_js(
    ...
    let inject_prelude = !input.replace('\\', "/").ends_with("/php_modules/core/prelude.phpx")
        && !input.replace('\\', "/").ends_with("/php_modules/core/prelude.js");
    let js = if let Some(program) = result.ast {
        match emit_js_from_ast_with_options(
            &program,
            source.as_bytes(),
            meta,
            EmitOptions { inject_prelude },
        ) {
            Ok(emitted) => emitted,
...
pub fn emit_js_from_ast(
    ...
    emit_js_from_ast_with_options(program, source, meta, EmitOptions::default())
}
...
pub fn emit_js_from_ast_with_options(
    program: &Program<'_>,
    source: &[u8],
    meta: SourceModuleMeta,
    options: EmitOptions,
) -> Result<String, String> {
    let mut emitter = JsSubsetEmitter::new(source, meta, options);
...
impl<'a> JsSubsetEmitter<'a> {
    fn new(source: &'a [u8], meta: SourceModuleMeta, options: EmitOptions) -> Self {
        Self {
            source,
            body: String::new(),
    ...
    fn finish(self) -> String {
        ...
        if self.inject_prelude {
            add_or_merge_import(
                ...
                "core/prelude",
                vec![ImportSpec {
                    imported: "install_globals".to_string(),
                    local: "__deka_install_globals".to_string(),
                }],
            );
        }
        ...
        if self.inject_prelude {
            out.push_str("__deka_install_globals();\n\n");
        }
