...
impl FileSystem for InMemoryFileSystem {
    ...
    fn remove(&self, path: &Path, options: RemoveOptions) -> Result<()> {
        ...
        if matches!(node.kind, NodeKind::Directory(_)) && !options.recursive && !node.is_dir_empty()
        {
...
fn get_parent_mut<'a>(root: &'a mut Node, components: &[String]) -> Result<(&'a mut Node, String)> {
    if components.is_empty() {
        return Err(invalid_input("path is empty"));
    }
    let name = components.last().unwrap().clone();
...
mod tests {
    ...
    fn write_and_read_file() {
        ...
        fs.mkdir(Path::new("/tmp"), MkdirOptions::default())
            .unwrap();
        fs.write_file(Path::new("/tmp/hello.txt"), b"hi", WriteOptions::default())
            .unwrap();
