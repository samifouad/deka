...
export type BridgeCallOutput =
  ...
  | { ok: false; error: string; code?: string; info?: BridgeErrorInfo };
...
export class PhpHostBridge {
  ...
  private readonly netAllowlist: string[];
  ...
  private normalizeFsAction(action: string): string {
    if (action === "read_file") return "readFile";
    if (action === "write_file") return "writeFile";
    if (action === "read_dir") return "readdir";
    return action;
  }
  ...
  private callNet(kind: string, action: string, payload: Record<string, unknown>): unknown {
    if (action === "fetch") {
  ...
  private isAllowedNetworkUrl(rawUrl: string): boolean {
...
function capabilityForBridgeCall(
  kind: string,
  action: string
