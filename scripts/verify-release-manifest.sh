...
manifest_php_wasm_sha="$(awk '/"php_wasm"/{f=1} f && /"sha256"/{gsub(/[", ]/,"",$2); print $2; exit}' "${MANIFEST}")"
...
if [[ "${php_wasm_sha}" != "${manifest_php_wasm_sha}" ]]; then
  echo "php wasm sha mismatch: manifest=${manifest_php_wasm_sha} actual=${php_wasm_sha}" >&2
  exit 1
fi
