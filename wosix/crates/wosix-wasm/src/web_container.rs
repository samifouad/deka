...
use wosix_core::{
    ...
    ProcessId, ProcessSignal, RemoveOptions, SpawnOptions, StdioMode, WatchOptions, WriteOptions,
    WosixError,
};
...
impl WebContainer {
    ...
    pub fn boot() -> WebContainer {
        WebContainer {
            ...
            process: Arc::new(InMemoryProcessHost::new()),
...
impl FsHandle {
    ...
    pub fn stat(&self, path: &str) -> Result<JsValue, JsValue> {
        ...
        let obj = Object::new();
        Reflect::set(&obj, &JsValue::from_str("size"), &JsValue::from_f64(meta.size as f64))?;
        Reflect::set(
            &obj,
...
fn event_to_js(event: FsEvent) -> Result<JsValue, JsValue> {
    let obj = Object::new();
    Reflect::set(&obj, &JsValue::from_str("path"), &JsValue::from_str(&event.path))?;
    Reflect::set(
        &obj,
...
fn exit_status_to_js(status: ExitStatus) -> Result<JsValue, JsValue> {
    let obj = Object::new();
    Reflect::set(&obj, &JsValue::from_str("code"), &JsValue::from_f64(status.code as f64))?;
...
fn controller_call(controller: &JsValue, method: &str, arg: Option<&JsValue>) -> Result<(), JsValue> {
...
fn port_info_to_js(info: PortInfo) -> Result<JsValue, JsValue> {
    let obj = Object::new();
    Reflect::set(&obj, &JsValue::from_str("port"), &JsValue::from_f64(info.port as f64))?;
    Reflect::set(&obj, &JsValue::from_str("url"), &JsValue::from_str(&info.url))?;
    Reflect::set(
        &obj,
...
fn port_event_to_js(event: PortEvent) -> Result<JsValue, JsValue> {
    ...
    match event {
        ...
        PortEvent::PortClosed(port) => {
            ...
            Reflect::set(&obj, &JsValue::from_str("port"), &JsValue::from_f64(port as f64))?;
        }
    }
    Ok(obj.into())
}
...
fn js_to_mount_tree(value: JsValue) -> Result<MountTree, JsValue> {
    ...
    if !value.is_object() {
        return Err(js_error("mount entry must be an object, string, or Uint8Array"));
    }
    let obj: Object = value
        .dyn_into()
