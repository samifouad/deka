...
function compileFrontmatterTemplateForWasm(source, mode) {
    if (mode === "php") {
        return source;
    }
...
function withModeMarker(source, mode) {
    ...
    if (trimmedNoBom.startsWith("/*__DEKA_PHPX__*/") ||
        trimmedNoBom.startsWith("/*__DEKA_PHPX_INTERNAL__*/")) {
        return source;
    }
    const marker = mode === "phpx_internal" ? "/*__DEKA_PHPX_INTERNAL__*/" : mode === "phpx" ? "/*__DEKA_PHPX__*/" : "";
    if (!marker) {
        return source;
    }
