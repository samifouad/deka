...
async function AsyncTitle(props) {
return jsx("h1", {"children": props.title});
}


const __phpx_main = async () => {
let sync = renderToString(jsx(Suspense, {"fallback": jsx("p", {"children": "Loading"}), "children": jsx(AsyncTitle, {"title": "Hello"})}));
globalThis.sync = sync;
(globalThis.__dekaPrint ? globalThis.__dekaPrint((sync + "\n")) : console.log((sync + "\n")));
let async = (await renderToStringAsync(jsx(Suspense, {"fallback": jsx("p", {"children": "Loading"}), "children": jsx(AsyncTitle, {"title": "Hello"})})));
