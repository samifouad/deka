...
async function Leaf(props) {
return jsx("span", {"children": props.label});
}

async function Box(props) {
return jsx("div", {"children": jsx(Leaf, {"label": props.label})});
}


const __phpx_main = async () => {
let sync = renderToString(jsx(Suspense, {"fallback": jsx("em", {"children": "Loading tree"}), "children": jsx(Box, {"label": "A"})}));
globalThis.sync = sync;
(globalThis.__dekaPrint ? globalThis.__dekaPrint((sync + "\n")) : console.log((sync + "\n")));
let async = (await renderToStringAsync(jsx(Suspense, {"fallback": jsx("em", {"children": "Loading tree"}), "children": jsx(Box, {"label": "A"})})));
globalThis.async = async;
(globalThis.__dekaPrint ? globalThis.__dekaPrint((async + "\n")) : console.log((async + "\n")));
};
await __phpx_main();
