...
export function strip_tags(string, allowed_tags) {
...
let allowed = __deka_parse_allowed_tags(allowed_tags);
let input = ("" + string);
...
let close = __deka_find_tag_end(input, (i + 1), len);
if ((close < 0)) {
(out += __deka_slice_bytes(input, i, len));
break;
}
let tagContent = __deka_slice_bytes(input, (i + 1), close);
let tagName = __deka_extract_tag_name(tagContent);
if (((tagName !== "") && ((allowed[tagName] !== undefined && allowed[tagName] !== null)))) {
(out += (("<" + tagContent) + ">"));
}
i = (close + 1);
}
...
if (globalThis.is_array(allowed_tags)) {
for (const tag of (Array.isArray(allowed_tags) ? allowed_tags : Object.values((allowed_tags ?? {})))) {
let name = __deka_extract_tag_name(("" + tag));
if ((name !== "")) {
(set[name] = true);
}
}
...
let text = ("" + allowed_tags);
let len = __deka_strlen_bytes(text);
let i = 0;
while ((i < len)) {
if ((text[i] !== "<")) {
(i += 1);
...
let end = __deka_find_tag_end(text, (i + 1), len);
if ((end < 0)) {
break;
}
let tagContent = __deka_slice_bytes(text, (i + 1), end);
let tagName = __deka_extract_tag_name(tagContent);
if ((tagName !== "")) {
(set[tagName] = true);
}
...
let len = __deka_strlen_bytes(tagContent);
let i = 0;
if ((len === 0)) {
return "";
}
...
let name = __deka_slice_bytes(tagContent, start, i);
