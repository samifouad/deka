...
let maskSet = __deka_trim_build_mask(mask);
let len = __deka_trim_strlen(value);
let start = 0;
let end = len;
if (trimLeft) {
while ((start < end)) {
let byte = globalThis.ord(value[start]);
if ((!((maskSet[byte] !== undefined && maskSet[byte] !== null)))) {
break;
}
(start += 1);
}
...
if (trimRight) {
while ((end > start)) {
let byte = globalThis.ord(value[(end - 1)]);
if ((!((maskSet[byte] !== undefined && maskSet[byte] !== null)))) {
break;
}
(end -= 1);
...
let set = [];
let len = __deka_trim_strlen(mask);
let i = 0;
while ((i < len)) {
let ch = mask[i];
if (((((i + 3) < len) && (mask[(i + 1)] === ".")) && (mask[(i + 2)] === "."))) {
let endCh = mask[(i + 3)];
let startByte = globalThis.ord(ch);
let endByte = globalThis.ord(endCh);
if ((startByte <= endByte)) {
for (let b = startByte; (b <= endByte); (b += 1)) {
(set[b] = true);
}
} else {
for (let b = startByte; (b >= endByte); (b -= 1)) {
