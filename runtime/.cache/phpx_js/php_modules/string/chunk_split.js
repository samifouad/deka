...
export function chunk_split(string, length, separator) {
if (length === undefined) { length = 76; }
...
let chunkLen = ((argc >= 2) ? Number.parseInt(length, 10) : 76);
if ((chunkLen < 1)) {
return null;
}
let sep = ((argc === 3) ? ("" + separator) : "\r\n");
let str = ("" + string);
...
for (let j = 0; ((j < chunkLen) && ((i + j) < len)); (j += 1)) {
(out += str[(i + j)]);
}
(out += sep);
}
