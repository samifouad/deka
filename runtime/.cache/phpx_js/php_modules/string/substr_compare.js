...
export function substr_compare(main_str, str, offset, length, case_insensitive) {
if (length === undefined) { length = null; }
if (case_insensitive === undefined) { case_insensitive = null; }
if ((globalThis.__phpx_func_num_args(arguments) < 3)) {
return null;
}
if ((globalThis.is_array(main_str) || globalThis.is_object(main_str))) {
return null;
}
if ((globalThis.is_array(str) || globalThis.is_object(str))) {
return null;
}
let main = ("" + main_str);
let cmp = ("" + str);
let lenMain = __deka_strlen_bytes(main);
...
let limit = null;
if ((length !== null)) {
limit = Number.parseInt(length, 10);
if ((limit < 0)) {
...
let segmentEnd = (start + limit);
if ((segmentEnd > lenMain)) {
segmentEnd = lenMain;
}
let segment = __deka_slice_bytes(main, start, segmentEnd);
if (case_insensitive) {
segment = __deka_to_lower_ascii(segment);
cmp = __deka_to_lower_ascii(cmp);
}
let segLen = __deka_strlen_bytes(segment);
let cmpLen = __deka_strlen_bytes(cmp);
for (let i = 0; (i < limit); (i += 1)) {
let b1 = ((i < segLen) ? globalThis.ord(segment[i]) : 0);
let b2 = ((i < cmpLen) ? globalThis.ord(cmp[i]) : 0);
if ((b1 !== b2)) {
return (b1 - b2);
}
}
