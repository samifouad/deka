...
export function str_split(string, length) {
...
let len = ((argc === 2) ? Number.parseInt(length, 10) : 1);
if ((len < 1)) {
return false;
}
let str = ("" + string);
let strLen = __deka_strlen_bytes(str);
let out = [];
if ((strLen === 0)) {
return out;
}
...
let chunk = "";
for (let j = 0; ((j < len) && ((i + j) < strLen)); (j += 1)) {
(chunk += str[(i + j)]);
}
(() => { const __arr = out; const __val = chunk; __arr.push(__val); return __val; })();
}
return out;
}
