...
if ((!globalThis.is_array(args))) {
return null;
}
let argCount = globalThis.count(args);
if ((argCount < 1)) {
return null;
}
if ((!globalThis.is_string(args[0]))) {
return null;
}
let format = args[0];
let formatLen = __deka_strlen_bytes(format);
let output = "";
let idx = 0;
let nextArg = 1;
while ((idx < formatLen)) {
...
(idx += 1);
let parsed = __deka_parse_format_spec(format, idx);
if ((parsed === null)) {
return null;
}
let spec = parsed["spec"];
(idx += parsed["consumed"]);
let argSlot = ((spec["position"] !== null) ? spec["position"] : nextArg);
...
let formatted = __deka_format_argument(args[argSlot], spec);
if ((formatted === null)) {
return null;
}
(output += formatted);
...
if ((!globalThis.is_array(values))) {
return null;
}
let args = [format];
for (const value of (Array.isArray(values) ? values : Object.values((values ?? {})))) {
(() => { const __arr = args; const __val = value; __arr.push(__val); return __val; })();
}
return args;
}
...
let cursor = start;
let len = __deka_strlen_bytes(format);
let spec = {"position": null, "left_align": false, "zero_pad": false, "show_sign": false, "space_sign": false, "width": null, "precision": null, "specifier": null};
if (((cursor < len) && __deka_is_digit(format[cursor]))) {
let lookahead = cursor;
let value = 0;
while (((lookahead < len) && __deka_is_digit(format[lookahead]))) {
value = ((value * 10) + (globalThis.ord(format[lookahead]) - 48));
(lookahead += 1);
}
...
hasWidth = true;
widthValue = ((widthValue * 10) + (globalThis.ord(format[cursor]) - 48));
(cursor += 1);
}
if (hasWidth) {
...
while (((cursor < len) && __deka_is_digit(format[cursor]))) {
hasPrecision = true;
precisionValue = ((precisionValue * 10) + (globalThis.ord(format[cursor]) - 48));
(cursor += 1);
}
(spec["precision"] = (hasPrecision ? precisionValue : 0));
}
while ((cursor < len)) {
let mod = format[cursor];
...
return {"spec": spec, "consumed": (cursor - start)};
}
...
let bytes = __deka_value_to_string(value);
if ((spec["precision"] !== null)) {
let limit = spec["precision"];
...
let raw = Number.parseInt(value, 10);
...
if ((__deka_strlen_bytes(digits) < precision)) {
digits = (globalThis.str_repeat("0", (precision - __deka_strlen_bytes(digits))) + digits);
}
}
}
let prefix = "";
...
let raw = __deka_coerce_unsigned(value);
let digits = __deka_int_to_base(raw, base, uppercase);
if ((spec["precision"] !== null)) {
let precision = spec["precision"];
if (((precision === 0) && (raw === 0))) {
digits = "";
...
let precision = ((spec["precision"] !== null) ? spec["precision"] : 6);
let formatted = globalThis.number_format(raw, precision, ".", "");
if ((raw >= 0)) {
if (spec["show_sign"]) {
formatted = ("+" + formatted);
} else {
...
if ((globalThis.is_int(value) || globalThis.is_float(value))) {
return ("" + value);
}
...
let len = __deka_strlen_bytes(value);
if ((len >= width)) {
return value;
}
let padding = globalThis.str_repeat(" ", (width - len));
...
if (spec["left_align"]) {
return (value + globalThis.str_repeat(" ", padLen));
}
if ((spec["zero_pad"] && (spec["precision"] === null))) {
if ((len > 0)) {
let first = value[0];
if ((((first === "-") || (first === "+")) || (first === " "))) {
return ((first + globalThis.str_repeat("0", padLen)) + __deka_slice_bytes(value, 1, len));
}
}
return (globalThis.str_repeat("0", padLen) + value);
}
return (globalThis.str_repeat(" ", padLen) + value);
}
...
let rem = (num % base);
out = (digits[rem] + out);
num = Number.parseInt((num / base), 10);
}
return out;
}
