...
export function levenshtein(string1, string2, cost_ins, cost_rep, cost_del) {
if (cost_ins === undefined) { cost_ins = null; }
if (cost_rep === undefined) { cost_rep = null; }
if (cost_del === undefined) { cost_del = null; }
...
let ins = ((cost_ins === null) ? 1 : Number.parseInt(cost_ins, 10));
let rep = ((cost_rep === null) ? 1 : Number.parseInt(cost_rep, 10));
let del = ((cost_del === null) ? 1 : Number.parseInt(cost_del, 10));
let left = ("" + string1);
let right = ("" + string2);
...
let current = [];
(current[0] = (i * del));
let leftByte = left[(i - 1)];
for (let j = 1; (j <= len2); (j += 1)) {
let cost = ((leftByte === right[(j - 1)]) ? 0 : rep);
let delCost = (prev[j] + del);
let insCost = (current[(j - 1)] + ins);
let repCost = (prev[(j - 1)] + cost);
let min = delCost;
if ((insCost < min)) {
min = insCost;
}
