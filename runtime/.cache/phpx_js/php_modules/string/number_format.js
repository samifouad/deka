...
export function number_format(number, decimals, decimal_separator, thousands_separator) {
if (decimals === undefined) { decimals = null; }
if (decimal_separator === undefined) { decimal_separator = null; }
if (thousands_separator === undefined) { thousands_separator = null; }
if ((globalThis.__phpx_func_num_args(arguments) < 1)) {
return null;
}
if ((globalThis.is_array(number) || globalThis.is_object(number))) {
return null;
}
let dec = 0;
if ((decimals !== null)) {
dec = Number.parseInt(decimals, 10);
if ((dec < 0)) {
dec = 0;
}
}
let decSep = ((decimal_separator === null) ? "." : ("" + decimal_separator));
let thouSep = ((thousands_separator === null) ? "," : ("" + thousands_separator));
...
let factor = 1;
if ((dec > 0)) {
factor = globalThis.pow(10, dec);
}
let rounded = Number.parseInt(globalThis.floor(((num * factor) + 0.5)), 10);
let intPart = ((dec > 0) ? Number.parseInt((rounded / factor), 10) : rounded);
let fracPart = ((dec > 0) ? Number.parseInt((rounded - (intPart * factor)), 10) : 0);
let intStr = ("" + intPart);
let intLen = __deka_strlen_bytes(intStr);
let grouped = "";
let groupCount = 0;
for (let i = (intLen - 1); (i >= 0); (i -= 1)) {
...
let fracLen = __deka_strlen_bytes(fracStr);
while ((fracLen < dec)) {
fracStr = ("0" + fracStr);
(fracLen += 1);
}
