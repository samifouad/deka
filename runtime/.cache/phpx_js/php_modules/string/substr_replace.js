...
export function substr_replace(string, replace, offset, length) {
...
if ((globalThis.is_array(replace) || globalThis.is_object(replace))) {
return null;
}
if ((globalThis.is_array(offset) || globalThis.is_object(offset))) {
return null;
}
if (((argc === 4) && (globalThis.is_array(length) || globalThis.is_object(length)))) {
return null;
}
let str = ("" + string);
let rep = ("" + replace);
...
let lenParam = Number.parseInt(length, 10);
if ((lenParam < 0)) {
end = (len + lenParam);
} else {
end = (start + lenParam);
...
let before = __deka_slice_bytes(str, 0, start);
let after = __deka_slice_bytes(str, end, len);
