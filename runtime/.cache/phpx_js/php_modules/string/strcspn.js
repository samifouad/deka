...
export function strcspn(string, mask, start, length) {
...
if ((globalThis.is_array(mask) || globalThis.is_object(mask))) {
return null;
}
let str = ("" + string);
let maskStr = ("" + mask);
let set = __deka_trim_build_mask(maskStr);
let len = __deka_strlen_bytes(str);
let startIndex = ((argc >= 3) ? Number.parseInt(start, 10) : 0);
if ((startIndex < 0)) {
...
let byte = globalThis.ord(str[i]);
if (((set[byte] !== undefined && set[byte] !== null))) {
break;
}
(count += 1);
}
