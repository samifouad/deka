...
export function count_chars(string, mode) {
...
let modeValue = ((argc === 2) ? Number.parseInt(mode, 10) : 0);
let input = ("" + string);
let counts = [];
let len = __deka_strlen_bytes(input);
for (let i = 0; (i < len); (i += 1)) {
let code = globalThis.ord(input[i]);
(counts[code] = (((counts[code] !== undefined && counts[code] !== null)) ? (counts[code] + 1) : 1));
}
if (((modeValue === 3) || (modeValue === 4))) {
let out = "";
for (let i = 0; (i < 256); (i += 1)) {
let present = ((counts[i] !== undefined && counts[i] !== null));
if ((((modeValue === 3) && present) || ((modeValue === 4) && (!present)))) {
(out += globalThis.chr(i));
}
}
return out;
}
