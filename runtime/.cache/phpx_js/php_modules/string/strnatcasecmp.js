...
export function strnatcasecmp(string1, string2) {
if ((globalThis.__phpx_func_num_args(arguments) !== 2)) {
return null;
}
if ((globalThis.is_array(string1) || globalThis.is_object(string1))) {
return null;
}
if ((globalThis.is_array(string2) || globalThis.is_object(string2))) {
return null;
}
let left = __deka_to_lower_ascii(("" + string1));
let right = __deka_to_lower_ascii(("" + string2));
...
let digits1 = __deka_slice_bytes(left, start1, i);
let digits2 = __deka_slice_bytes(right, start2, j);
let cmp = __deka_compare_digit_chunks(digits1, digits2);
if ((cmp !== 0)) {
return cmp;
}
continue;
...
return (globalThis.ord(c1) - globalThis.ord(c2));
}
if ((i < len1)) {
return 1;
}
...
let byte = globalThis.ord(ch);
return ((byte >= 48) && (byte <= 57));
}

function __deka_compare_digit_chunks(left, right) {
let trimLeft = __deka_trim_leading_zeros(left);
let trimRight = __deka_trim_leading_zeros(right);
let lenLeft = __deka_strlen_bytes(trimLeft);
let lenRight = __deka_strlen_bytes(trimRight);
if ((lenLeft !== lenRight)) {
return (lenLeft - lenRight);
}
for (let i = 0; (i < lenLeft); (i += 1)) {
let b1 = globalThis.ord(trimLeft[i]);
let b2 = globalThis.ord(trimRight[i]);
if ((b1 !== b2)) {
return (b1 - b2);
}
}
