...
let total = 0;
let result = __deka_replace_dispatch(search, replace, subject, false, total);
if ((argc === 4)) {
count = total;
}
return result;
...
let searchList = (globalThis.is_array(search) ? search : [search]);
let replaceList = (globalThis.is_array(replace) ? replace : null);
let replaceScalar = (globalThis.is_array(replace) ? null : replace);
if (globalThis.is_array(subject)) {
let out = [];
for (const [key , value] of Object.entries(subject)) {
(out[key] = __deka_replace_in_string(("" + value), searchList, replaceList, replaceScalar, caseInsensitive, count));
}
return out;
}
return __deka_replace_in_string(("" + subject), searchList, replaceList, replaceScalar, caseInsensitive, count);
}

function __deka_replace_in_string(subject, searchList, replaceList, replaceScalar, caseInsensitive, count) {
let out = subject;
let index = 0;
for (const search of (Array.isArray(searchList) ? searchList : Object.values((searchList ?? {})))) {
let rep = ((replaceScalar !== null) ? replaceScalar : (((replaceList[index] !== undefined && replaceList[index] !== null)) ? replaceList[index] : ""));
...
let out = "";
let pos = 0;
let needleLen = __deka_strlen_bytes(needle);
let subjectLen = __deka_strlen_bytes(subject);
while ((pos <= subjectLen)) {
let found = __deka_strpos_bytes(haystack, needle, pos);
if ((found < 0)) {
