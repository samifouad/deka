...
if ((globalThis.is_array(first) || globalThis.is_object(first))) {
return null;
}
if ((globalThis.is_array(second) || globalThis.is_object(second))) {
return null;
}
let left = ("" + first);
let right = ("" + second);
let score = __deka_similar_text(left, right);
if ((argc === 3)) {
...
if ((sum > 0)) {
if (((pos1 > 0) && (pos2 > 0))) {
(sum += __deka_similar_text(__deka_slice_bytes(left, 0, pos1), __deka_slice_bytes(right, 0, pos2)));
}
if ((((pos1 + max) < len1) && ((pos2 + max) < len2))) {
(sum += __deka_similar_text(__deka_slice_bytes(left, (pos1 + max), len1), __deka_slice_bytes(right, (pos2 + max), len2)));
}
}
return sum;
}
