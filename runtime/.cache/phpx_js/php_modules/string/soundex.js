...
let input = __deka_to_upper_ascii(("" + string));
...
let first = input[0];
let out = first;
let prevCode = __deka_soundex_code(first);
for (let i = 1; ((i < len) && (__deka_strlen_bytes(out) < 4)); (i += 1)) {
let code = __deka_soundex_code(input[i]);
