...
export function __deka_html_escape(value, encodeQuotes, doubleEncode) {
if (encodeQuotes === undefined) { encodeQuotes = true; }
if (doubleEncode === undefined) { doubleEncode = true; }
let input = ("" + value);
...
let match = __deka_html_match_entity(input, i, len);
if (match["found"]) {
(out += match["value"]);
i = match["next"];
continue;
...
let content = __deka_slice_bytes(input, (start + 1), end);
...
for (let i = 0; (i < __deka_strlen_bytes(content)); (i += 1)) {
let byte = globalThis.ord(content[i]);
...
let map = {"&lt;": "<", "&gt;": ">", "&quot;": "\"", "&#039;": "'", "&apos;": "'", "&amp;": "&"};
for (const [entity , value] of Object.entries(map)) {
let entityLen = __deka_strlen_bytes(entity);
if (((start + entityLen) <= len)) {
if ((__deka_slice_bytes(input, start, (start + entityLen)) === entity)) {
return {"found": true, "value": value, "next": (start + entityLen)};
}
}
}
return {"found": false, "value": "", "next": (start + 1)};
}
