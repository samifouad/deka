...
export function str_pad(string, length, pad_string, pad_type) {
...
let str = ("" + string);
let target = Number.parseInt(length, 10);
let current = __deka_strlen_bytes(str);
if ((target <= current)) {
return str;
}
let padStr = ((argc >= 3) ? ("" + pad_string) : " ");
if ((padStr === "")) {
return false;
}
let padType = ((argc === 4) ? Number.parseInt(pad_type, 10) : 1);
...
if ((padType === 2)) {
left = Number.parseInt((padLen / 2), 10);
right = (padLen - left);
} else {
right = padLen;
}
...
let out = "";
let padLen = __deka_strlen_bytes(pad);
if ((padLen === 0)) {
return "";
}
let i = 0;
