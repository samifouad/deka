...
export function strtr(string, from, to) {
...
if (((argc === 2) && globalThis.is_array(from))) {
let keys = [];
let vals = [];
...
(() => { const __arr = keys; const __val = keyStr; __arr.push(__val); return __val; })();
(() => { const __arr = vals; const __val = ("" + v); __arr.push(__val); return __val; })();
}
let count = globalThis.count(keys);
if ((count === 0)) {
return str;
}
...
let key = keys[i];
let klen = __deka_strlen_bytes(key);
...
if ((globalThis.is_array(from) || globalThis.is_object(from))) {
return null;
}
if (((argc === 3) && (globalThis.is_array(to) || globalThis.is_object(to)))) {
return null;
}
let fromStr = ("" + from);
let toStr = ((argc === 3) ? ("" + to) : "");
let fromLen = __deka_strlen_bytes(fromStr);
if ((fromLen === 0)) {
return str;
}
let toLen = __deka_strlen_bytes(toStr);
let map = [];
let limit = ((fromLen < toLen) ? fromLen : toLen);
