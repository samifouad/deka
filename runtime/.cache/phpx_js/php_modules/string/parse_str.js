...
export function parse_str(string, result) {
if (result === undefined) { result = null; }
let args = globalThis.__phpx_func_get_args(arguments);
let argc = globalThis.count(args);
if (((argc < 1) || (argc > 2))) {
return null;
}
let input = ("" + string);
let root = [];
let pairs = __deka_parse_query_pairs(input);
for (const pair of (Array.isArray(pairs) ? pairs : Object.values((pairs ?? {})))) {
let rawKey = pair[0];
if ((rawKey === "")) {
continue;
}
let key = __deka_urldecode_bytes(rawKey);
let val = __deka_urldecode_bytes(pair[1]);
let parsed = __deka_parse_key_segments(key);
let base = parsed[0];
if ((base === "")) {
continue;
}
...
if ((i > start)) {
let part = __deka_slice_bytes(input, start, i);
if ((part !== "")) {
let eq = __deka_strpos_bytes(part, "=", 0);
if ((eq >= 0)) {
(() => { const __arr = pairs; const __val = [__deka_slice_bytes(part, 0, eq), __deka_slice_bytes(part, (eq + 1), __deka_strlen_bytes(part))]; __arr.push(__val); return __val; })();
} else {
(() => { const __arr = pairs; const __val = [part, ""]; __arr.push(__val); return __val; })();
}
}
}
start = (i + 1);
...
if (((iByte === "%") && ((i + 2) < len))) {
let hi = __deka_hex_value(value[(i + 1)]);
let lo = __deka_hex_value(value[(i + 2)]);
if (((hi >= 0) && (lo >= 0))) {
(out += globalThis.chr(((hi << 4) + lo)));
(i += 3);
continue;
}
}
...
let base = "";
let segments = [];
let len = __deka_strlen_bytes(key);
let i = 0;
while (((i < len) && (key[i] !== "["))) {
(base += key[i]);
(i += 1);
...
let content = __deka_slice_bytes(key, start, i);
if ((content === "")) {
(() => { const __arr = segments; const __val = null; __arr.push(__val); return __val; })();
} else {
(() => { const __arr = segments; const __val = content; __arr.push(__val); return __val; })();
}
(i += 1);
}
...
let baseKey = __deka_array_key_from_string(base);
let segCount = globalThis.count(segments);
if ((segCount === 0)) {
(root[baseKey] = value);
return;
}
if (((!((root[baseKey] !== undefined && root[baseKey] !== null))) || (!globalThis.is_array(root[baseKey])))) {
(root[baseKey] = []);
}
let current = root[baseKey];
for (let idx = 0; (idx < segCount); (idx += 1)) {
let segment = segments[idx];
let isLast = (idx === (segCount - 1));
if (isLast) {
if ((segment === null)) {
(() => { const __arr = current; const __val = value; __arr.push(__val); return __val; })();
} else {
let key = __deka_array_key_from_string(segment);
(current[key] = value);
}
return;
}
if ((segment === null)) {
(() => { const __arr = current; const __val = []; __arr.push(__val); return __val; })();
let lastKey = globalThis.array_key_last(current);
current = current[lastKey];
continue;
}
let key = __deka_array_key_from_string(segment);
if (((!((current[key] !== undefined && current[key] !== null))) || (!globalThis.is_array(current[key])))) {
(current[key] = []);
}
current = current[key];
}
