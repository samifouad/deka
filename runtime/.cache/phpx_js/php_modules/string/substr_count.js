...
if ((globalThis.is_array(haystack) || globalThis.is_object(haystack))) {
return null;
}
if ((globalThis.is_array(needle) || globalThis.is_object(needle))) {
return null;
}
let hs = ("" + haystack);
let nd = ("" + needle);
...
let hlen = __deka_strlen_bytes(hs);
let start = ((argc >= 3) ? Number.parseInt(offset, 10) : 0);
if ((start < 0)) {
start = (hlen + start);
}
if ((start < 0)) {
...
let segment = __deka_slice_bytes(hs, start, end);
let segLen = __deka_strlen_bytes(segment);
let needleLen = __deka_strlen_bytes(nd);
...
let pos = 0;
while (true) {
let found = __deka_strpos_bytes(segment, nd, pos);
if ((found < 0)) {
break;
}
(count += 1);
