...
if ((globalThis.is_array(to) || globalThis.is_object(to))) {
return null;
}
let input = ("" + string);
let fromStr = ("" + from);
...
let fromLabel = __deka_cyrillic_label(fromStr[0]);
let toLabel = __deka_cyrillic_label(toStr[0]);
...
return globalThis.mb_convert_encoding(input, toLabel, fromLabel);
}

function __deka_cyrillic_label(code) {
let byte = globalThis.ord(code);
if (((byte >= 65) && (byte <= 90))) {
(byte += 32);
}
switch (byte) {
