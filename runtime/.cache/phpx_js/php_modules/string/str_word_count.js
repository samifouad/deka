...
export function str_word_count(string, format, charlist) {
...
let fmt = ((argc >= 2) ? Number.parseInt(format, 10) : 0);
let extra = ((argc === 3) ? ("" + charlist) : "");
let input = ("" + string);
...
let word = __deka_slice_bytes(input, start, i);
if ((fmt === 0)) {
(count += 1);
} else {
if ((fmt === 1)) {
(() => { const __arr = words; const __val = word; __arr.push(__val); return __val; })();
} else {
if ((fmt === 2)) {
(words[start] = word);
} else {
...
let len = __deka_strlen_bytes(extra);
