...
if ((globalThis.is_array(characters) || globalThis.is_object(characters))) {
return null;
}
let str = ("" + string);
let mask = __deka_trim_build_mask(("" + characters));
let len = __deka_strlen_bytes(str);
let out = "";
for (let i = 0; (i < len); (i += 1)) {
let ch = str[i];
let byte = globalThis.__deka_ord(ch);
if ((!((mask[byte] !== undefined && mask[byte] !== null)))) {
(out += ch);
continue;
}
if ((byte === 10)) {
...
let v = (value & 255);
let a = Number.parseInt((v / 64), 10);
let b = Number.parseInt(((v % 64) / 8), 10);
let c = (v % 8);
return ((("" + a) + b) + c);
}
