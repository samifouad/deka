...
let names = [];
for (const arg of (Array.isArray(args) ? args : Object.values((args ?? {})))) {
if (globalThis.is_array(arg)) {
for (const value of (Array.isArray(arg) ? arg : Object.values((arg ?? {})))) {
(() => { const __arr = names; const __val = ("" + value); __arr.push(__val); return __val; })();
}
} else {
if ((((globalThis.is_string(arg) || globalThis.is_int(arg)) || globalThis.is_float(arg)) || globalThis.is_bool(arg))) {
(() => { const __arr = names; const __val = ("" + arg); __arr.push(__val); return __val; })();
}
}
}
let out = [];
for (const name of (Array.isArray(names) ? names : Object.values((names ?? {})))) {
if (globalThis.__deka_symbol_exists(name, 2)) {
(out[name] = globalThis.__deka_symbol_get(name, 2));
}
}
return out;
}
