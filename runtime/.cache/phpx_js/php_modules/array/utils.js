...
for (const [k , _] of Object.entries(array)) {
if (((k === key) || (k === key))) {
return true;
}
}
...
export function __deka_array_sort_preserve_keys(array, descending) {
...
globalThis.sort(values);
if (descending) {
values = globalThis.array_reverse(values);
}
let buckets = [];
let indices = [];
for (const [key , value] of Object.entries(array)) {
let marker = __deka_array_value_to_string(value);
if ((!((buckets[marker] !== undefined && buckets[marker] !== null)))) {
(buckets[marker] = []);
(indices[marker] = 0);
}
(() => { const __arr = buckets[marker]; const __val = key; __arr.push(__val); return __val; })();
}
let out = [];
...
let index = indices[marker];
let key = buckets[marker][index];
