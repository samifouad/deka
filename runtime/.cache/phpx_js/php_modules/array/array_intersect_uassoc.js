...
export function array_intersect_uassoc(array) {
...
let callback = args[(argc - 1)];
let others = [];
for (let i = 1; (i < (argc - 1)); (i += 1)) {
let arr = args[i];
if ((!globalThis.is_array(arr))) {
return null;
}
(() => { const __arr = others; const __val = arr; __arr.push(__val); return __val; })();
}
let out = [];
for (const [key , value] of Object.entries(array)) {
let valStr = __deka_array_value_to_string(value);
let found = false;
for (const other of (Array.isArray(others) ? others : Object.values((others ?? {})))) {
for (const [otherKey , otherValue] of Object.entries(other)) {
if ((__deka_array_value_to_string(otherValue) !== valStr)) {
continue;
}
if ((Number.parseInt(globalThis.call_user_func(callback, key, otherKey), 10) === 0)) {
found = true;
break;
}
}
