...
let keys = [];
for (const [key , _] of Object.entries(array)) {
...
let pick = ((argc === 2) ? Number.parseInt(num, 10) : 1);
if (((pick < 1) || (pick > count))) {
return null;
}
if ((pick === 1)) {
return keys[globalThis.rand(0, (count - 1))];
}
for (let i = (count - 1); (i > 0); (i -= 1)) {
let j = globalThis.rand(0, i);
let tmp = keys[i];
(keys[i] = keys[j]);
(keys[j] = tmp);
}
...
(() => { const __arr = out; const __val = keys[i]; __arr.push(__val); return __val; })();
}
return out;
}
