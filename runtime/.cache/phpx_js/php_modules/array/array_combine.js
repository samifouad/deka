...
if (((!globalThis.is_array(keys)) || (!globalThis.is_array(values)))) {
return null;
}
let keyList = [];
for (const key of (Array.isArray(keys) ? keys : Object.values((keys ?? {})))) {
(() => { const __arr = keyList; const __val = key; __arr.push(__val); return __val; })();
}
let valueList = [];
for (const value of (Array.isArray(values) ? values : Object.values((values ?? {})))) {
(() => { const __arr = valueList; const __val = value; __arr.push(__val); return __val; })();
}
if ((globalThis.count(keyList) !== globalThis.count(valueList))) {
return false;
}
let out = [];
let len = globalThis.count(keyList);
for (let i = 0; (i < len); (i += 1)) {
let key = __deka_array_key_from_value(keyList[i]);
if ((key === null)) {
return null;
}
(out[key] = valueList[i]);
