...
export function array_chunk(array, length, preserve_keys) {
...
let size = Number.parseInt(length, 10);
if ((size <= 0)) {
return null;
}
let preserve = ((argc === 3) ? Boolean(preserve_keys) : false);
let out = [];
let chunk = [];
let chunkIndex = 0;
let chunkLen = 0;
...
let useKey = (preserve ? key : chunkPos);
(chunk[useKey] = value);
(chunkPos += 1);
(chunkLen += 1);
if ((chunkLen === size)) {
