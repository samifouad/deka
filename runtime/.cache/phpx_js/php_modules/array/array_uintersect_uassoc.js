...
export function array_uintersect_uassoc(array) {
...
let valueCallback = args[(argc - 2)];
let keyCallback = args[(argc - 1)];
let out = [];
for (const [key , value] of Object.entries(array)) {
let found = false;
for (let i = 1; (i < (argc - 2)); (i += 1)) {
let arr = args[i];
if ((!globalThis.is_array(arr))) {
continue;
}
for (const [otherKey , otherValue] of Object.entries(arr)) {
if ((Number.parseInt(globalThis.call_user_func(valueCallback, value, otherValue), 10) !== 0)) {
continue;
}
if ((Number.parseInt(globalThis.call_user_func(keyCallback, key, otherKey), 10) === 0)) {
found = true;
break;
}
}
