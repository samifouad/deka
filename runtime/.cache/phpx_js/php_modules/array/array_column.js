...
export function array_column(array, column_key, index_key) {
if (index_key === undefined) { index_key = null; }
...
let colKey = __deka_array_key_from_value(column_key);
if ((colKey === null)) {
return [];
}
let useIndex = (argc === 3);
let indexKey = (useIndex ? __deka_array_key_from_value(index_key) : null);
let out = [];
for (const row of (Array.isArray(array) ? array : Object.values((array ?? {})))) {
if ((!globalThis.is_array(row))) {
continue;
}
...
let outKey = __deka_array_key_from_value(row[indexKey]);
if ((outKey !== null)) {
(out[outKey] = value);
continue;
}
