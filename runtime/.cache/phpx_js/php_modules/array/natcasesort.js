...
(() => { const __arr = items; const __val = {"key": key, "value": value}; __arr.push(__val); return __val; })();
}
let count = globalThis.count(items);
let i = 1;
while ((i < count)) {
let current = items[i];
let j = (i - 1);
while ((j >= 0)) {
let cmp = globalThis.strnatcasecmp(("" + items[j]["value"]), ("" + current["value"]));
if ((cmp <= 0)) {
break;
}
(items[(j + 1)] = items[j]);
