...
export function bytes_slice(bytes, start, length) {
...
let n = strlen(bytes);
for (let i = 0; (i < n); (i++)) {
(() => { const __arr = out; const __val = ord(bytes[i]); __arr.push(__val); return __val; })();
}
return out;
}
...
let out = "";
for (const v of (Array.isArray(arr) ? arr : Object.values((arr ?? {})))) {
let b = Number.parseInt(v, 10);
if ((b < 0)) {
b = 0;
}
if ((b > 255)) {
