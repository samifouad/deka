...
function sqlExec(db, rawSql, params) {
...
function sqlQuery(db, rawSql, params) {
    const sql = normalizeSql(rawSql);
    ...
    if (literal) {
        ...
        for (const field of fields) {
            ...
            if (m)
                row[m[2]] = parseValueToken(m[1], params, cursor);
            else
                row[field] = parseValueToken(field, params, cursor);
        }
        return [row];
    }
...
function parseWhere(expr, params) {
...
function parseValueToken(tokenRaw, params, cursor) {
    const token = tokenRaw.trim();
    if (token === "?")
        return params[cursor.i++] ?? null;
    const numbered = token.match(/^\$(\d+)$/);
    if (numbered)
        return params[Math.max(0, Number(numbered[1]) - 1)] ?? null;
...
function normalizeDriver(kind, payloadDriver) {
...
function dbNameFromConfig(config, driver) {
    const fromConfig = [config.database, config.dbname, config.name, config.path, config.file].find((value) => typeof value === "string" && String(value).trim().length > 0);
    if (fromConfig)
